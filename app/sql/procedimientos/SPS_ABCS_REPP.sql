...
BEGIN
	...
	IF @ACCION = 'B'
	BEGIN
		DELETE FROM REPP WHERE PARAMETROID=@PARAMETROID AND REPORTEID=CASE WHEN ISNULL(@REPORTEID,-1)>0 THEN @REPORTEID ELSE REPORTEID END
	END
	IF @ACCION = 'C'
	BEGIN
		UPDATE REPP SET 
...
--DELETE FROM REPP WHERE PARAMETROID=11
