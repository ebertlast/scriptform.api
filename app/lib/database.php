...
class Database
{
    ...
    public static function StartUpLote($dbhost = '', $dbname = '', $dbuser = '', $dbpasswd = '')
    {
        try {
            $settings = require '../src/settings.php';
            $conf = $settings["settings"];
            if ($dbhost == '') {$dbhost = $conf['database_lotes']['dbhost'];}
            if ($dbname == '') {$dbname = $conf['database_lotes']['dbname'];}
            if ($dbuser == '') {$dbuser = $conf['database_lotes']['dbuser'];}
            if ($dbpasswd == '') {$dbpasswd = $conf['database_lotes']['dbpasswd'];}
